...
dnl AC_CHECK_TOOL(AR, ar, :)
	...
	if test $tcl_ok = usable; then
	    ...
	    AC_MSG_RESULT([$tcl_ok])
	    if test $tcl_ok = usable; then
		AC_DEFINE(USE_A_OUT_H,1,[Define if your system has a.out.h])
	    else
		...
		AC_MSG_RESULT([$tcl_ok])
		if test $tcl_ok = usable; then
		    AC_DEFINE(USE_SYS_EXEC_AOUT_H,1,[Define if your machine has <sys/exec_aout.h>])
		else
		    DL_OBJS=""
		fi
	    fi
...
#include <sys/dirent.h>],[struct dirent64 p;],
	...
	if test "x${tcl_cv_struct_dirent64}" = "xyes" ; then
	    AC_DEFINE(HAVE_STRUCT_DIRENT64,1,[Define if sys/types.h defines struct dirent64])
	fi
	...
	if test "x${tcl_cv_struct_stat64}" = "xyes" ; then
	    AC_DEFINE(HAVE_STRUCT_STAT64,1,[Define if your machines defines struct stat64])
	fi
	...
	if test "x${tcl_cv_type_off64_t}" = "xyes" ; then
	    AC_DEFINE(HAVE_TYPE_OFF64_T,1,[Define if your sys/types.h defines off64_t])
	fi
